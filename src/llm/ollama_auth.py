"""Auth helpers for Ollama (local and Ollama Cloud)."""
...
def build_ollama_headers(api_key: Optional[str]) -> Dict[str, str]:
    """Return request headers, adding a bearer token when an API key is set."""
    if not api_key:
        return {}
...
def validate_ollama_auth(base_url: Optional[str], api_key: Optional[str]) -> None:
    """Ollama Cloud requires an API key — fail fast with a clear message."""
